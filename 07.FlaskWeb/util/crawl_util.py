...
def get_bestseller():
    ...
    for li in lis: 
        ...
        href = li.select_one('.coverImage > label > a')['href']     # 자식 selector
        src = li.select_one('.coverImage img')['src']               # 자손 selector
        title = li.select_one('.itemName').get_text().strip()
        author = li.select_one('.author').get_text().strip()
        company = li.select_one('.company').get_text().strip()
        price = li.select_one('.price > em').get_text().strip()
        price = int(price.replace(',',''))
        data.append({'순위': rank, '제목': title, '저자': author, '출판사': company, '가격': f'{price:7,d}', 'href':base_url+href, 'src':src})
