...
def calc():
    if request.method == 'GET':
        ...
        result = eval(f'{num1}{op}{num2}')      #eval(): 2+3을 넣으면 5가 나오는 함수
        return f'''
...
def lang():
    ...
    if request.method == 'GET':
        ...
        foods = request.form.getlist('food')        # getlist() 에서 key값인 food를 넣어준다
        selected_food = []
        for food_index in foods:
            selected_food.append(food_list[int(food_index)])

        return f'''
            <h2>선택한 언어: {lang_dict[language]}</h2>
            <h2>선택한 음식: {selected_food}</h2>
